...
static __always_inline int check_ratelim_ipv4(__u32 req_ip, __u64 now) {
    struct buckets *req_buck =
        bpf_map_lookup_elem(&ratelimit_per_ipv4, &req_ip);
    ...
    if (!req_buck) {
        ...
        // inserting into map
        bpf_map_update_elem(&ratelimit_per_ipv4, &req_ip, &new_bucket,
                            BPF_NOEXIST);
    ...
    if (new_tokens > 0) {
        ...
        req_buck->last_updated_at += new_tokens * NS_PER_TOKEN;
    }
...
static __always_inline int check_ratelim_ipv6(__u8 req_ip[16], __u64 now) {
    ...
    if (!req_buck) {
        ...
        // inserting into map
        bpf_map_update_elem(&ratelimit_per_ipv6, req_ip, &new_bucket,
                            BPF_NOEXIST);
...
int filter(struct xdp_md *ctx) {
    ...
    // ipv6
        ...
        if ((void *)(ip6 + 1) > data_end)
            return XDP_PASS;
        // bpf_printk("ipv6 packet received"); //printing the entire thing is
        // pain, plus we'll have to remove this anyways for benchmarking
        struct lpm_keyipv6 key6 = {};
        key6.prefixlen = 128;
        __builtin_memcpy(key6.ip, ip6->saddr.in6_u.u6_addr8, 16);

        int block_status =
            check_block(bpf_map_lookup_elem(&lpm_map_ipv6, &key6), now);
        if (block_status == XDP_DROP) {
            return XDP_DROP;
        }
