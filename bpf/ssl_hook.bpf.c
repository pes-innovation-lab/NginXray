...
int BPF_URETPROBE(ssl_read_exit) {
    ...
    bpf_printk("lookup ssl=%llx", e->ssl_ptr);
    ...
    bpf_printk("read_exit client=%x:%u server=%x:%u", e->client_ip,
               e->client_port, e->server_ip, e->server_port);
...
int BPF_URETPROBE(ssl_write_exit) {
    ...
    bpf_printk("write_exit client=%x:%u server=%x:%u", e->client_ip,
               e->client_port, e->server_ip, e->server_port);
    bpf_ringbuf_submit(e, 0);
    bpf_map_delete_elem(&bufs, &tid);
    return 0;
...
int BPF_UPROBE(ssl_set_fd, void *ssl, int fd) {
    ...
    bpf_printk("ssl=%llx fd=%d mapped", ssl_key, fd);
